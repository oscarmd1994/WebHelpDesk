document.addEventListener('DOMContentLoaded', () => {
    ...
    const bodyRegional = document.getElementById('body-regional');
            ...
            $.ajax({
                ...
                beforeSend: () => {
                    ...
                    if (data.Bandera == true && data.MensajeError == "none") {
                        ...
                        localStorage.setItem("countDbRegional", quantity);
                        ...
                        bodyRegional.innerHTML += data.Html;
                        if (quantity > 0) {
    ...
    fImportDataRegionalSip = (paramint, paramstr1) => {
        try {
            ...
            setTimeout(() => {
                ...
                $.ajax({
                    url: "../ImportBD/ImportRegionalSIP",
                    type: "POST",
                    data: { quantity: parseInt(localStorage.getItem("countDbRegional")), option: parseInt(paramint) },
                    beforeSend: () => {
                        icoShowLoad.classList.add('text-primary');
                    }, success: (data) => {
                        console.log(data);
                        if (paramint == 1) {
                            if (data.CorrectoInsert === true || data.Bandera === true) {
                            ...
                            if (data.CorrectoUpdate === true || data.Busqueda === true) {
                                dataBdDateSip.textContent = "";
                                let downloadFile = "";
                                setTimeout(() => {
                                    if (data.BanderaArchivo == true) {
    ...
    btnInsertData.addEventListener('click', () => {
        ...
        `;
        fImportDataRegionalSip(1, 'Insertando información existente en el WS...');
    });
    ...
    btnUpdateData.addEventListener('click', () => {
        ...
        btnUpdateData.innerHTML = `
            <span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span> Espere por favor...
        `;
        fImportDataRegionalSip(2, 'Actualizando información existente en el WS...');
    });
