document.addEventListener('DOMContentLoaded', () => {
    ...
    const bodyDepartaments = document.getElementById('body-departaments');
    ...
    // Funcion que consulta los datos para llenar la tabla de departamentos
    fExtractionOfInformationOfDepartaments = () => {
            ...
            $.ajax({
                ...
                beforeSend: () => {
                    ...
                    if (data.Bandera == true && data.MensajeError == "none") {
                        let count = 0;
                        localStorage.setItem("countDbDepto", data.Datos);
    ...
    fImportDataDepartamentsSip = (paramint, paramstr1) => {
        try {
            ...
            setTimeout(() => {
                ...
                $.ajax({
                    url: "../ImportBD/ImportDepartamentsSIP",
                    type: "POST",
                    data: { quantity: parseInt(localStorage.getItem("countDbDepto")), option: parseInt(paramint) },
                    beforeSend: () => {
                        icoShowLoad.classList.add('text-primary');
                    }, success: (data) => {
                        console.log(data);
                        if (data.Bandera === true) {
                            if (paramint == 1) {
                                if (data.Insercion === true || data.Busqueda === true || data.Coincidencia === true) {
                                    dataBdDateSip.textContent = "";
                                    let downloadFile = "";
                                    setTimeout(() => {
                            ...
                            btnInsertData.innerHTML = '<i class="fas fa-list-alt mr-2"></i> Registrar datos';
                            btnUpdateData.innerHTML = `<i class="fas fa-undo mr-2"></i> Actualizar datos`;
                            document.getElementById('content-msj-error').innerHTML += `
                               <div class="card shadow rounded border-left-danger border-right-danger p-2">
                                    <h6 class="card-title mt-3 text-center text-danger">
                                        <i class="fas fa-exclamation-triangle mr-2"></i> Opss! Ha ocurrio una excepción...
                                    </h6>
                                    <h6 class="text-center">Favor de indicar este mensaje al área de TI.</h6>
                                    <small class="text-center">${data.MensajeError}</small>
                                </div>  
                            `;
                            document.getElementById('content-btn-restart').innerHTML += `
                                <button class="btn btn-light text-primary shadow btn-sm font-weight-bold" 
    ...
    btnInsertData.addEventListener('click', () => {
        btnInsertData.disabled  = true;
        btnUpdateData.disabled  = true;
        btnInsertData.innerHTML = `<span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span> Espere por favor...`;
        fImportDataDepartamentsSip(1, 'Insertando información existente en el WS...');
    });

    btnUpdateData.addEventListener('click', () => {
        btnUpdateData.disabled  = true;
        btnInsertData.disabled  = true;
        btnUpdateData.innerHTML = `<span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span> Espere por favor...  `;
        fImportDataDepartamentsSip(2, 'Actualizando información existente en el WS...');
    });
