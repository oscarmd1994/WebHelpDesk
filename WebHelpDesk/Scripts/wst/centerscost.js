document.addEventListener('DOMContentLoaded', () => {
    ...
    const bodyCenters = document.getElementById('body-centerscost');
    ...
    fExtractionOfInformationOfCentersCost = () => {
            ...
            $.ajax({
                ...
                beforeSend: () => {
                    ...
                    if (data.Bandera == true && data.MensajeError == "none") {
                        ...
                        localStorage.setItem("countDbCentersC", quantity);
    ...
    fImportDataCentersCostSip = (paramint, paramstr1) => {
        try {
            ...
            setTimeout(() => {
                ...
                $.ajax({
                    url: "../ImportBD/ImportCentersCostSIP",
                    type: "POST",
                    data: { quantity: parseInt(localStorage.getItem("countDbCentersC")), option: parseInt(paramint) },
                    beforeSend: () => {
                        icoShowLoad.classList.add('text-primary');
                    }, success: (data) => {
                        console.log(data);
                        if (paramint == 1) {
                            if (data.Insercion === true || data.Busqueda === true) {
                                fExtractionOfInformationOfCentersCost();
                                //dataBdDateSip.textContent = "";
                                let downloadFile = "";
                                setTimeout(() => {
    ...
    btnInsertData.addEventListener('click', () => {
        ...
        `;
        fImportDataCentersCostSip(1, 'Insertando información existente en el WS...');
    });
    ...
    btnUpdateData.addEventListener('click', () => {
        ...
        btnUpdateData.innerHTML = `
            <span class="spinner-grow spinner-grow-sm" role="status" aria-hidden="true"></span> Espere por favor...
        `;
        fImportDataCentersCostSip(2, 'Actualizando información existente en el WS...');
    });
